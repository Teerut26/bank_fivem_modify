...
Citizen.CreateThread(function()
    while true do
        ...
        if playerNearBank() or playerNearATM() then
            DisplayHelpText('<font face="font4thai"> กด [ ~r~E ~w~] เพื่อเปิดธนาคาร </font>')
    ...
    while true do
        ...
        if not blipsLoaded then
            for k, v in ipairs(bankLocations) do
		        ...
		        AddTextComponentString('<font face="font4thai"> ธนาคาร </font>')
...
RegisterNUICallback('deposit', function(data)
    if not atATM then
        ...
        TriggerEvent("pNotify:SendNotification",  {
            text =  "คุณไม่สามารถฝากเงินในธนาคารได้",
            type = "info",
            timeout = 3500,
            layout = "topRight",
        })
